...
최신 신조어 5문항 + 레트로 유행어 5문항, 총 10문항(약 90초)으로 내 **언어 나이**와 세대
라벨을 판독하는 재미 진단. eden appworks / fineboll 서비스.
...
lib/shinjo-engine/   결정적 엔진 (프레임워크 무의존)
  ...
  paper.ts           generatePaper(최신5+레트로5) · newSeed · parseSeed
  age.ts             read() 언어 나이 · 세대 라벨 · 갭 판독
lib/content/         items(40문항) · copy(문구 90) · select · guides(3)
...
## 언어 나이 로직 (constants.ts 에서 조정)

기준 30세에서 시작 → 클램프 15~65.

| | 정답 | 오답 |
|---|---|---|
| 최신 신조어 | 젊어짐 −2 (2023+ 표현 −3) | 늙어짐 +2 |
| 레트로 유행어 | 늙어짐 +2 (~2008 +3, ~2000 +4) | 젊어짐 −2 |

`eraYear` 가중으로 양끝 세대(15세 잘파어 원어민 / 60세 사전파)가 도달 가능. 같은 답안 →
항상 같은 나이(결정적).
